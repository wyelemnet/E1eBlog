...
使用[URL.createObjectURL()](https://developer.mozilla.org/zh-CN/docs/Web/API/URL/createObjectURL)创建读取后的流文件url;
...
three.js的TDSLoader加载3ds模型(其他类型模型使用其他loader):
    ...
    // 修改url, 比如一个3ds模型的zip文件中, 会有纹理图片等, 但是使用URL.createObjectURL()创建的url与原本的不对应, 所有存储		在fileMap中并在加载模型时修改url
...
### problems
```
1. zip.js报错: jszip.min.js:13 Uncaught (in promise) Error: The constructor with parameters has been removed in JSZip 3.0, please check the upgrade guide.
  3.0版本之前:
